	...
	switch (section) {
		...
		case "mint":
			if (page) {
				...
				APP.loader.show();
				$.get("data/" + section + "/" + page + ".json", function (data) {
					setTimeout(function () { APP.mintDetail.load(data); }, 800);
					if (storeHistory) { history.pushState(state, title, p + state); }
					APP.state = state;
					document.title = data['page-title'];
					APP.webGL.go("mintDetail");
					setTimeout(function () { APP.loader.update(.7); }, 500);
					setTimeout(function () { APP.mintDetail.show(); APP.loader.update(1.0); APP.loader.hide() }, 1000);
				});

			} else {
				// art index
				var title = APP.data.mint.title;
				document.title = APP.data.mint["page-title"];
				setTimeout(function () { APP.mint.show(); }, 0);
				APP.webGL.go("mint");
				if (storeHistory) { history.pushState(state, title, p + state); }
				APP.state = state;
			}
